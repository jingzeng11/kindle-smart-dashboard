...
The physical KT3 calibration identifies `/dev/input/event0` (`zforce2`) with direct 600 × 800 coordinates. Handwriting is always active in the main content area; no mode switch is required. The footer exposes three quiet text actions: **撤销**, **清空**, and **阅读**. Touch points are buffered and the Kindle's built-in `eips` submits exactly one screen refresh when each stroke ends, avoiding a backlog of low-level update requests. Strokes are stored only in `data/handwriting.bin`, are redrawn after every network refresh, and are preserved by both USB and SSH client upgrades. After KUAL closes and the stock Home screen is stable, the client captures its raw framebuffer before drawing the dashboard. **阅读** restores those exact pixels and resumes the paused processes without restarting the Kindle framework, avoiding white screens, dashboard remnants, and disruption to later KUAL actions. Handwriting is never uploaded or converted into Apple Calendar data.
